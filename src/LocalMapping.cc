    ...
    for(size_t i=0; i<vpNeighKFs.size(); i++)
    {
        ...
        for(int ikp=0; ikp<nmatches; ikp++)
        {
            ...
            if(!bStereo1) //单目
            {
                ...
                if((errX1*errX1+errY1*errY1)>5.991*sigmaSquare1)// 基于卡方检验计算出的阈值（假设测量有一个像素的偏差）
                    continue;
            }
            else // stereo
            {
            ...
            /*if(fabs(ratioDist-ratioOctave)>ratioFactor)
                continue;*/
            // ratioDist*ratioFactor < ratioOctave 或 ratioDist/ratioOctave > ratioFactor TODO: 表明尺度变化是连续的?
...
cv::Mat LocalMapping::ComputeF12(KeyFrame *&pKF1, KeyFrame *&pKF2)
{
    ...
    const cv::Mat &K1 = pKF1->mK; //一般同一相机内参一样
