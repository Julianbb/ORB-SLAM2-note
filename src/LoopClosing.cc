...
//在local mapping中调用，插入关键帧进行loop detection
...
{
    //步骤１： 从队列里取出候选关键帧进行　loop detection
    {
        unique_lock<mutex> lock(mMutexLoopQueue);
    ...
    // VIII-A
    // 步骤3：遍历所有共视关键帧，计算当前关键帧与每个共视关键的bow相似度得分，并得到最低得分minScore
    ...
    for(size_t i=0, iend=vpCandidateKFs.size(); i<iend; i++)
    {
        ...
        set<KeyFrame*> spCandidateGroup = pCandidateKF->GetConnectedKeyFrames();//得到与该关键帧连接的关键帧
        spCandidateGroup.insert(pCandidateKF); // 组成　一个子候选组
        ...
        for(size_t iG=0, iendG=mvConsistentGroups.size(); iG<iendG; iG++)
        {
            ...
            if(bConsistent)
            {
                ...
                if(!vbConsistentGroup[iG])
                {
                    ...
                    vbConsistentGroup[iG]=true; //避免重复，this avoid to include the same group more than once
                }
...
bool LoopClosing::ComputeSim3()
{
    ...
    vector<vector<MapPoint*> > vvpMapPointMatches; // 保存匹配mappoint点
    ...
    for(int i=0; i<nInitialCandidates; i++)
    {
        ...
        pKF->SetNotErase();// 防止在LocalMapping中KeyFrameCulling函数将此关键帧作为冗余帧剔除
        ...
        // 步骤2：将当前帧mpCurrentKF与闭环候选关键帧匹pKF配,vvpMapPointMatches是匹配特征点对应的MapPoints
        int nmatches = matcher.SearchByBoW(mpCurrentKF,pKF,vvpMapPointMatches[i]);

        if(nmatches<20)
        {
            vbDiscarded[i] = true;// 匹配的特征点数太少，该候选帧剔除
            continue;
        }
        else
        {
    ...
    while(nCandidates>0 && !bMatch)
    {
        for(int i=0; i<nInitialCandidates; i++)
        {
            ...
            KeyFrame* pKF = mvpEnoughConsistentCandidates[i]; //通过了上面步骤ｃulling的结果
            ...
            if(!Scm.empty())
            {
                ...
                {
                    if(vbInliers[j])// 保存inlier的MapPoints
...
void LoopClosing::CorrectLoop()
{
...

    {
        ...
        for(vector<KeyFrame*>::iterator vit=mvpCurrentConnectedKFs.begin(), vend=mvpCurrentConnectedKFs.end(); vit!=vend; vit++)
        {
            ...
            if(pKFi!=mpCurrentKF)
            {
                ...
                CorrectedSim3[pKFi]=g2oCorrectedSiw;// 得到闭环g2o优化后各个关键帧的位姿(只是由当前帧变换传递得到，并不是真的每帧进行优化)
            }
    ...
    for(vector<KeyFrame*>::iterator vit=mvpCurrentConnectedKFs.begin(), vend=mvpCurrentConnectedKFs.end(); vit!=vend; vit++)
    {
        ...
        // 步骤5.5：从连接关系中去除闭环之前的二级连接关系，剩下的连接就是由闭环得到的新连接关系
        for(vector<KeyFrame*>::iterator vit_prev=vpPreviousNeighbors.begin(), vend_prev=vpPreviousNeighbors.end(); vit_prev!=vend_prev; vit_prev++)
        {
            LoopConnections[pKFi].erase(*vit_prev);
        }
        // 步骤5.6：从连接关系中去除闭环之前的一级连接关系，剩下的连接就是由闭环得到的新连接关系
...
void LoopClosing::RunGlobalBundleAdjustment(unsigned long nLoopKF)
{
    ...
    // 跟新地图
    {
        ...
        if(!mbStopGBA)
        {
            ...
            while(!mpLocalMapper->isStopped() && !mpLocalMapper->isFinished()) //等带localMapper完成
            {
                usleep(1000);
            }
